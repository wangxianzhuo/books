...
public class PaginationTag extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        ...
        if (current > 1) {
            out.println("<li><a class='pagination-url' href='" + baseUrl + (hasParam ? "&" : "?") + "page=" + (current - 1) + "'>上一页</a></li>");
        } else {
        ...
        if (totalPage <= 10) {
            for (int i = 1; i <= totalPage; i++) {
                if (current != i) {
                    out.println("<li><a class='pagination-url' href='" + baseUrl + (hasParam ? "&" : "?") + "page=" + i + "'>"
                            + i + "</a></li>");
                } else {
            ...
            for (int i = totalPage - 1; i <= totalPage; i++) {
                out.println("<li><a class='pagination-url' href='" + baseUrl + (hasParam ? "&" : "?") + "page=" + i + "'>" + i
                        + "</a></li>");
            }
        ...
        if (totalPage > current) {
            out.println("<li><a class='pagination-url' href='" + baseUrl + (hasParam ? "&" : "?") + "page=" + (current + 1)
                    + "'>下一页</a></li>");
        } else {
